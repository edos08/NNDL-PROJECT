...
def find_classes(directory: Union[str, Path], hierarchy: int = 0) -> Tuple[List[str], Dict[str, int]]:
    ...
    if hierarchy == 0: # use directories in root as classes (default implementation)
        classes = sorted(entry.name for entry in os.scandir(directory) if entry.is_dir())

    if hierarchy == 1: # descend one folder hierarchy to create classes
        classes = []
    ...
    if hierarchy == 2: # descend two folder hierarchies
        classes = []
...
class HierarchicalDatasetFolder(VisionDataset):
    ...
    def find_classes(self, directory: Union[str, Path]) -> Tuple[List[str], Dict[str, int]]:
        ...
        """
        return find_classes(directory, self.hierarchy)
...
class CompCarsImageFolder(HierarchicalDatasetFolder):
