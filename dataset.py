...
class ImagesFromTextFile(VisionDataset):
    ...
    def __init__(self,
        ...
        samples = self.make_dataset(self.root, txt_file, class_to_idx)  # read samples from textfile with correct class index as target
...
# Same as ImageFolder class, except for overriden find_classes method.
    ...
    def find_classes(self, directory: str | Path) -> Tuple[List[str], Dict[str, int]]:          
        ...
        # NOTE: hierarchy == 2 fails due to the fact that some years are not well defined (e.g empty) in CompCars dataset
